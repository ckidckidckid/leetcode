...
class Solution:
    def orderOfLargestPlusSign(self, N, mines):
        ...
        """
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
        # Brute Force O(N^2 + mines)
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #

        grid = [[1]*N for _ in range(N)]
        for mine in mines:
            grid[mine[0]][mine[1]] = 0
        # count of U,D,L,R 1's in the grid
        table = [[[0,0,0,0] for _ in range(N)] for _ in range(N)]
        ans = 0
        for i in range(N):
            for j in range(N):
                if i>0:
                    table[i][j][0] = (1+table[i-1][j][0]) if grid[i-1][j]==1 else 0
                if j>0:
                    table[i][j][2] = (1+table[i][j-1][2]) if grid[i][j-1]==1 else 0
        for i in range(N-1,-1,-1):
            for j in range(N-1,-1,-1):
                if i<N-1:
                    table[i][j][1] = (1+table[i+1][j][1]) if grid[i+1][j]==1 else 0
                if j<N-1:
                    table[i][j][3] = (1+table[i][j+1][3]) if grid[i][j+1]==1 else 0

        for i in range(N):
            for j in range(N):
                if grid[i][j] == 1:
                    ans = max(ans, 1 + min(table[i][j]))
        return ans
