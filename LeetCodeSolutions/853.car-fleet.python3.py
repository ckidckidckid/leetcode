...
class Solution:
    def carFleet(self, target, position, speed):
        ...
        if n==0:
            return 0
        composite = sorted(list(zip(position, speed)))
        position, speed = zip(*composite)
        p,s = position[-1], speed[-1]
        for i in range(n-2, -1, -1):
            if (speed[i] >= s and position[i] == p) or (speed[i] > s and ((p*speed[i] - position[i]*s)/(speed[i]-s)) <= target):
                ans -= 1
            else:
